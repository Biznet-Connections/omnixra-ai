...
router.post("/:id/message", protect, async (req, res) => {
  try {
    ...
    try {
      const recipient = conversation.participants.find(p => String(p) !== String(req.user._id));
      if (recipient) {
        notifyNewMessage(recipient, req.user.name || "New message", (req.body.text || "").slice(0, 80), conversation._id).catch(() => {});
      }
    } catch (e) { /* silent */ }
