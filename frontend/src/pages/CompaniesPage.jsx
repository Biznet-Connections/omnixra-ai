...
function readCompaniesCache() {
  try {
    ...
    if (Date.now() - parsed.t > COMPANIES_CACHE_TTL) return null;
    return parsed;
  } catch (e) { return null; }
}

function writeCompaniesCache(payload) {
  try {
...
function CompaniesPage() {
  ...
  const [cachedHadData, setCachedHadData] = useState(!!(cached?.companies?.length || cachedCompanies?.length));
